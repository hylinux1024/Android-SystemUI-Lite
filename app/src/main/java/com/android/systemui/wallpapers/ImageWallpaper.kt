...
/**
 * Minimal WallpaperService that draws the current system wallpaper.
 *
 * Faithful standalone port of AOSP SystemUI's
 * src/com/android/systemui/wallpapers/ImageWallpaper.java CanvasEngine —
 * ported from the proven SystemUI-Lite2 implementation
 * (com.android.systemui.wallpapers.ImageWallpaper).
 *
 * This service is NOT started at boot. It is bound by the system's
 * WallpaperManagerService ONLY after the user selects it in the system
 * wallpaper picker (Settings → Wallpaper → Live wallpapers → this service).
 * It therefore cannot crash the persistent SystemUI boot path.
 *
 * Draw pipeline (mirrors AOSP drawFrameOnCanvas):
 *   onSurfaceChanged → loadWallpaper()  → get WallpaperManager.drawable → bitmap
 *   onVisibilityChanged(visible) / onSurfaceRedrawNeeded → drawFrame()
 *     surface.lockHardwareCanvas() → canvas.drawBitmap(bmp, null, surfaceFrame, null)
 */
    ...
    inner class ImageWallpaperEngine : Engine() {
        ...
        private fun loadWallpaper() {
            ...
            try {
                ...
                try {
                    // AOSP fallback: clear to default wallpaper and retry.
                    wm.clear()
                    val drawable = wm.drawable
                    if (drawable != null) {
                        val bmp = drawableToBitmap(drawable)
        ...
        private fun drawFrame() {
            ...
            // Bind a non-null canvas; bail out if neither hardware nor software lock works.
            val canvas: Canvas = try {
                surface.lockHardwareCanvas()
            } catch (e: Exception) {
                try {
                    // Hardware canvas unsupported on this surface; fall back to software.
